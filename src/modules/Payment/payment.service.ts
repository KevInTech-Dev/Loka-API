...
export class PaymentService {
    ...
    async createPayment(userId: string, role: string, paymentData: CreatePaymentInput): Promise<PaymentResponse> {
        ...
        const { tenantId, landlordId, amountPaid, factureForeignKeys } = await this.resolvePaymentContext(
            paymentData.facture_type,
            facture,
            tenant?.id,
            landlord?.id,
        ...
        const selectedProvider = paymentData.payment_provider || PaymentProviderEnum.FEDAPAY;
        const selectedCurrency = paymentData.currency || DEFAULT_CURRENCY;
        this.assertProviderCurrencyCompatibility(selectedProvider, selectedCurrency);
    ...
    async getPaymentById(id: string, userId: string, role: string): Promise<PaymentResponse> {
        ...
        const factureId = this.getFactureIdFromPayment(payment.factureType, payment);
        const factureDetails = factureId
            ? await this.paymentRepository.getFactureByTypeAndId(payment.factureType, factureId)
            : null;
    ...
    private async resolvePaymentContext(
        ...
        switch (factureType) {
            ...
            case InvoiceType.FACTURE_ELEC: {
                ...
                if (!tenantIdFromUser || data.idTenant !== tenantIdFromUser) {
                    throw new UnauthorizedError("You cannot pay this electricity invoice");
                }
                const contract = await this.getActiveContractByTenant(tenantIdFromUser);
                return {
                    tenantId: tenantIdFromUser,
            ...
            case InvoiceType.FACTURE_MAINTENANCE: {
                ...
                if (!tenantIdFromUser || data.idTenant !== tenantIdFromUser) {
                    throw new UnauthorizedError("You cannot pay this maintenance invoice");
                }
                const contract = await this.getActiveContractByTenant(tenantIdFromUser);
                return {
                    tenantId: tenantIdFromUser,
            ...
            case InvoiceType.ABONNEMENT_TRIAL: {
                ...
                if (!landlordIdFromUser || data.landlordId !== landlordIdFromUser) {
                    throw new UnauthorizedError("You cannot pay this subscription invoice");
                }
    ...
    private getFactureIdFromPayment(type: InvoiceType, payment: Payment): string | null {
        switch (type) {
            case InvoiceType.FACTURE_LOYER:
    ...
    private async generatePaymentReference(): Promise<string> {
        ...
        const nextNumber = (await this.paymentRepository.countByYearMonth(year, month)) + 1;
